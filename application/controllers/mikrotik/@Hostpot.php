...
class Hostpot extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($q <> '') {
			$config['base_url'] = base_url() . 'index.php/mikrotik/Hostpot/index.html?q=' . urlencode($q);
			$config['first_url'] = base_url() . 'index.php/mikrotik/Hostpot/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'index.php/mikrotik/Hostpot/index/';
			$config['first_url'] = base_url() . 'index.php/mikrotik/Hostpot/index/';
		}

		$config['per_page'] = 100;
		$config['page_query_string'] = FALSE;
		$config['total_rows'] = $this->Hostpot_model->total_rows($q);
		$Hostpot = $this->Hostpot_model->get_limit_data($config['per_page'], $start, $q);
		...
		$data = array(
			'hostpot_data' => $Hostpot,
			'q' => $q,
			'pagination' => $this->pagination->create_links(),
			'total_rows' => $config['total_rows'],
			'start' => $start,
		);
		$this->template->load('template', 'Hostpot/Hostpot_list', $data);
	}

	// public function read($id)
	// {
	// 	$row = $this->Hostpot_model->get_by_id($id);
	// 	if ($row) {
	// 		$Hostpot = $row->id_Hostpot;
	...
	// 						'create_date' => date('y-m-d H:i:s')
	// 					);
	// 					$this->Hostpot_model->update($id_hostpot, $Hostpot, $data_hostpot);
	// 				}
	// 			}
	// 			// PPP
	// 			$API->write('/ppp/secret/print');
	...
	// 						'create_date' => date('y-m-d H:i:s')
	// 					);
	// 					$this->Ppp_model->update($id_ppp, $Hostpot, $data_hostpot);
	// 				}
	// 			}
	// 			$API->disconnect();
	// 			redirect(site_url('mikrotik/Hostpot'));
	// 		} else {
	// 			redirect(site_url('mikrotik/Hostpot'));
	// 		}
	// 	} else {
	...
	public function create()
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Create',
				'action' => site_url('mikrotik/hostpot/create_action'),
				'id_hostpot' => set_value('id_hostpot'),
			...
			$data['profile_data'] = $this->HostpotProfile_model->tampil_profileHostpot();
			$this->template->load('template', 'hostpot/hostpot_form', $data);
		}
	}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($row) {
				...
				if ($API->connect($host, $username, $password)) {
					...
					$this->Hostpot_model->insert($data);
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('mikrotik/hostpot/update_action'),
				'id' => set_value('id', $row->id),
				'id_hostpot' => set_value('id_hostpot', $row->id_hostpot),
				'server' => set_value('server', $row->server),
				'name' => set_value('name', $row->name),
				'password' => set_value('password', $row->password),
				'profile' => set_value('profile', $row->profile),
				'disabled' => set_value('disabled', $row->disabled),
			...
			$data['profile_data'] = $this->HostpotProfile_model->tampil_profileHostpot();
			$data['data_hostpot'] = $this->Hostpot_model->edit_data($id);
			$this->template->load('template', 'hostpot/hostpot_form', $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('mikrotik/hostpot'));
	...
	public function update_action()
	{
		$this->_rules();
		if ($this->form_validation->run() == FALSE) {
			$this->update($this->input->post('id_hostpot', TRUE));
		} else {
			...
			if ($rowMikrotik) {
				...
				if ($API->connect($host, $username, $password)) {
					...
					$this->Hostpot_model->update($this->input->post('id_hostpot', TRUE), $data);
	...
	public function excel()
	{
		...
		xlsWriteLabel($tablehead, $kolomhead++, "Nama Hostpot");
		xlsWriteLabel($tablehead, $kolomhead++, "Alamat");
		xlsWriteLabel($tablehead, $kolomhead++, "User Hostpot");
		xlsWriteLabel($tablehead, $kolomhead++, "Password Hostpot");
		xlsWriteLabel($tablehead, $kolomhead++, "Ip Hostpot");
		...
		foreach ($this->Hostpot_model->get_all() as $data) {
			$kolombody = 0;

			//ubah xlsWriteLabel menjadi xlsWriteNumber untuk kolom numeric
			xlsWriteNumber($tablebody, $kolombody++, $nourut);
			xlsWriteLabel($tablebody, $kolombody++, $data->Hostpot);
			xlsWriteLabel($tablebody, $kolombody++, $data->alamat);
			xlsWriteLabel($tablebody, $kolombody++, $data->user_Hostpot);
			xlsWriteLabel($tablebody, $kolombody++, $data->pass_Hostpot);
			xlsWriteLabel($tablebody, $kolombody++, $data->ip_Hostpot);
	...
	public function word()
	{
		...
		header("Content-Disposition: attachment;Filename=Hostpot.doc");

		$data = array(
			'Hostpot_data' => $this->Hostpot_model->get_all(),
			'start' => 0
		);

		$this->load->view('Hostpot/Hostpot_doc', $data);
	}
	...
	public function active()
	{
		...
		if ($row) {
			...
			if ($API->connect(
				...
				// Hostpot
				// $users = $API->comm('/ip/hotspot/active/getall');
				$API->write('/ip/hotspot/active/getall', false);
				$API->write('=.proplist=.id,server,user,address,login-by,bytes-in,bytes-out,packets-in,packets-out,comment');
				$hostpot = $API->read();
		...
		$this->template->load('template', 'Hostpot/Hostpot_aktif', $data);
	}
	public function deleteAktif($id)
	{
		...
		if ($rowMikrotik) {
			...
			if ($API->connect($host, $username, $password)) {
				...
				$API->disconnect();
				$this->session->set_flashdata('message', 'Delete Record Success');
				redirect(site_url('mikrotik/hostpot/active'));
			}
		} else {
			redirect(site_url('mikrotik/hostpot/active'));
		}
		// } else {
		// 	$this->session->set_flashdata('message', 'a Not Found');
		// 	redirect(site_url('mikrotik/hostpot/aktif'));
		// }
	}
}

/* End of file Hostpot.php */
/* Location: ./application/controllers/Hostpot.php */

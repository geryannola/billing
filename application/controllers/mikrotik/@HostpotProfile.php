...
class HostpotProfile extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($q <> '') {
			$config['base_url'] = base_url() . 'index.php/mikrotik/HostpotProfile/index.html?q=' . urlencode($q);
			$config['first_url'] = base_url() . 'index.php/mikrotik/HostpotProfile/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'index.php/mikrotik/HostpotProfile/index/';
			$config['first_url'] = base_url() . 'index.php/mikrotik/HostpotProfile/index/';
		}

		$config['per_page'] = 100;
		$config['page_query_string'] = FALSE;
		$config['total_rows'] = $this->HostpotProfile_model->total_rows($q);
		$HostpotProfile = $this->HostpotProfile_model->get_limit_data($config['per_page'], $start, $q);
		...
		$data = array(
			'hostpot_data' => $HostpotProfile,
			'q' => $q,
			'pagination' => $this->pagination->create_links(),
			'total_rows' => $config['total_rows'],
			'start' => $start,
		);
		$this->template->load('template', 'HostpotProfile/HostpotProfile_list', $data);
	}

	// public function read($id)
	// {
	// 	$row = $this->HostpotProfile_model->get_by_id($id);
	// 	if ($row) {
	// 		$HostpotProfile = $row->id_HostpotProfile;
	...
	// 						'comment' => $comment,
	// 						'id_HostpotProfile' => $HostpotProfile,
	// 						'is_aktive' => 1,
	...
	// 						'create_date' => date('y-m-d H:i:s')
	// 					);
	// 					$this->HostpotProfile_model->update($id_profile_hostpot, $HostpotProfile, $data_hostpot);
	// 				}
	// 			}
	// 			// PPP
	// 			$API->write('/ppp/secret/print');
	...
	// 						'HostpotProfiles' => $p["HostpotProfiles"],
	...
	// 						'create_date' => date('y-m-d H:i:s')
	// 					);
	// 					$this->Ppp_model->update($id_ppp, $HostpotProfile, $data_hostpot);
	// 				}
	// 			}
	// 			$API->disconnect();
	...
	public function create()
	{
		...
		$data['parent_queue'] = $this->HostpotProfile_model->tampil_parentQueue();
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($row) {
				...
				if ($API->connect(
					...
					$this->HostpotProfile_model->insert($data);
	...
	public function update($id)
	{
		...
		if ($row) {
			...
			$data['data_hostpot'] = $this->HostpotProfile_model->edit_data($id);
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->HostpotProfile_model->update($this->input->post('id_profile_hostpot', TRUE), $data);
	...
	public function delete($id)
	{
		...
		if ($row) {
			$this->HostpotProfile_model->delete($id);
	...
	public function excel()
	{
		...
		$namaFile = "HostpotProfile.xls";
		$judul = "HostpotProfile";
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		//penulisan header
		...
		xlsWriteLabel($tablehead, $kolomhead++, "Nama HostpotProfile");
		xlsWriteLabel($tablehead, $kolomhead++, "Alamat");
		xlsWriteLabel($tablehead, $kolomhead++, "User HostpotProfile");
		xlsWriteLabel($tablehead, $kolomhead++, "Password HostpotProfile");
		xlsWriteLabel($tablehead, $kolomhead++, "Ip HostpotProfile");
		...
		foreach ($this->HostpotProfile_model->get_all() as $data) {
			$kolombody = 0;

			//ubah xlsWriteLabel menjadi xlsWriteNumber untuk kolom numeric
			xlsWriteNumber($tablebody, $kolombody++, $nourut);
			xlsWriteLabel($tablebody, $kolombody++, $data->HostpotProfile);
			xlsWriteLabel($tablebody, $kolombody++, $data->alamat);
			xlsWriteLabel($tablebody, $kolombody++, $data->user_HostpotProfile);
			xlsWriteLabel($tablebody, $kolombody++, $data->pass_HostpotProfile);
			xlsWriteLabel($tablebody, $kolombody++, $data->ip_HostpotProfile);
	...
	public function word()
	{
		...
		header("Content-Disposition: attachment;Filename=HostpotProfile.doc");

		$data = array(
			'HostpotProfile_data' => $this->HostpotProfile_model->get_all(),
			'start' => 0
		);

		$this->load->view('HostpotProfile/HostpotProfile_doc', $data);
	}
}

/* End of file HostpotProfile.php */
/* Location: ./application/controllers/HostpotProfile.php */
